...
async def amain(args: argparse.Namespace) -> None:
    ...
    try:
                    ...
                    try:
                        async def live_checkpoint(checkpoint_no):
                            ...
                            if built_paths and os.getenv("GITHUB_ACTIONS", "").lower() == "true":
                                push_ok = await asyncio.get_running_loop().run_in_executor(
                                    None,
        ...
        if args.rebuild:
            ...
            dirty_movies = set(state.dirty) | set(
                getattr(locals().get("stats", None), "affected_movies", set())
                if "stats" in locals() else set()
            )

            # ONE-TIME REPOSITORY BOOTSTRAP:
            # A movie can already exist in SQLite/state but still have no
            # movie-slug JSON in the checked-out repository. Build it once.
            # On later runs, the file exists and is skipped unless dirty.
            missing_movies = []
            ...
            to_build = sorted(dirty_movies | set(missing_movies))
        ...
        if args.rebuild:
            log.info(
                "Movies total=%d dirty=%d missing_bootstrap=%d to_build=%d",
                len(all_movies), len(state.dirty), 0, len(to_build),
            )
        else:
            log.info(
                "Movies total=%d dirty=%d missing_bootstrap=%d to_build=%d",
                len(all_movies), len(state.dirty), len(missing_movies), len(to_build),
            )
            if missing_movies:
                log.info(
                    "Bootstrap rule: %d movie JSON files do not exist in repo/local output; "
                    "building them once now.",
                    len(missing_movies),
                )
        ...
        if not args.rebuild:
            log.info("Missing JSONs bootstrapped : %d", len(set(missing_movies) - failed_builds))
